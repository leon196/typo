...
void main() {
	...
	float radius = .1*pow(elapsed, 0.5);//*random(uv+vec2(9.5247));
	...
	for (float other = 0.; other < countmax; ++other) {
		...
		if (length(uvmap-uv) > 0.001) {
			...
			follow += dirOther * smoothstep(0.5,1.,distOther);
		}
	}
	...
	// velocity += twist;
	// velocity += curl * .1;
	velocity += vec3(0,-1,0) * 1.;// * (1.-collide.y);
	velocity += normalize(-pos) * collide * 4.;
	// velocity += grany * .1;
